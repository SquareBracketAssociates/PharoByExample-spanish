...
\begin{itemize}
	...
	\item{El \menu{Process Browser} permite ver todos los procesos(o \emph{threads}) que se est\'an ejecutando en Smalltalk.}
...
exactamente lo mismo. Se ver\'a la relaci\'on precisa en \secref{packages}}, pero puede cambiar esto tambi\'en si as\'i lo desea.  Si ya tiene el navegador centrado en la clase de la que desea una
...
clases existentes por medio del uso sus nombres.  \index{class!creation}
...
Estas herramientas pueden ser accedidas \actclick{ing} en varios de los men\'ues
...
La existencia de las \ind{versiones del navegador} significa que nunca tendr\'as
...
El change set est\'a descrito en \secref{env:changeSet}}
...
del paquete, como se muestra en \figref{MC+PBE}.  Esto esta bien: todav\'ia
puedes guardar el c\'odigo, lo que causar\'a que se escriba el el package-cache.
Simplemente haz clic en el bot\'on \button{Save} y ser\'as invitado a proporcionar un registro de la versi\'on del paquete que est\'as a punto de guardar, como se muestra en \figref{PBE-on}; cuando aceptas el mensaje, Monticello guardar\'a tu paquete. Para indicar esto, el asterisco decorando el nombre en el paquete en el panel de Monticello ser\'a eliminado, y se agregar\'a el n\'umero de la versi\'on.
...
necesario que te asegures primero que Monticello sepa sobre el repositorio, agreg\'andolo si es necesario. Entonces cuando usas el bot\'on \button{Copy} en el navegador del repositorio de package-cache, y seleccionas el repositorio al cual el paquete deber\'ia ser copiado.
...
entonces usando el bot\'on \button{Copy} desde el navegador package-cache.
...
pen\'ultima l\'inea del m\'etodo para encontrar el \ct{punto} en el string \ct{'readme.txt'}, y que la ejecuci\'on nunca deber\'ia haber llegado a la \'ultima l\'inea. \pharo no nos permite correr una ejecuci\'on hacia atr\'as, pero esto nos permite comenzar un m\'etodo de nuevo, que funciona muy bien en el c\'odigo, como este que no muta objetos, pero en su lugar crea otros nuevos.
...
Los botones \button{Into} y \button{Over} nos brindan dos diferentes maneras de
paso a trav\'es de la ejecuci\'on.  Si hace clic en el bot\'on \button{Over},
\pharo ejecuta el actual env\'io de mensaje (en este caso el \ct{do:}) en un
paso, a menos que exista un error. As\'i \button{Over} nos llevar\'a al pr\'oximo env\'io-mensaje en el actual m\'etodo, el cual es \ct{value}\,---\,este est\'a exactamente donde comenzamos, y no ayudan mucho. Lo que debemos hacer es averiguar porque el \ct{do:} no est\'a encontrando el caracter que estamos buscando.
...
\dothis{Clic \button{Into}; \pharo ingresar\'a en el m\'etodo correspondiente al env\'io de mensaje resaltado, en este caso, \ct{Collection>>>do:}.}

Sin embargo, resulta que esto no es una gran ayuda: podemos estar bastante
seguros de que  \ct{Collection>>>do:} no es lo que rompe.  El error es mucho
m\'as probable que est\'e en \emph{que} estamos pidiendo hacer a \pharo. \button{Through} es el bot\'on apropiado para utilizar en este caso: queremos ignorar los detalles de \ct{do:} en s\'i y centrarse en la ejecuci\'on del argumento del bloque.
...
Cuando \ct{each} es \ct{7} esperamos que el bloque \ct{ifTrue:} sea ejecutado,
pero no es as\'i. Para ver qu\'e es lo que va mal, vaya a \button{Into} la ejecuci\'on de \ct{value:} como se ilustra en \figref{steppingIntoValue}.
...
\begin{figure}[btp]
	...
	\end{center}
	\caption{Cambiando el m\'etodo \ct{suffix} en el debugger: corrigiendo el error
...
abrir un depurador en el c\'alculo escribiendo \short{.} (eso es una detenci\'on completa o un per\'iodo, dependiendo donde aprendi\'o Ingles).\footnote{Tambi\'en es \'util saber que puede hacer aparecer un depurador de emergencia en cualquier momento escribiendo \short{{\sc shift--}.}} Tambi\'en puedes solo editar el c\'odigo sospechoso a insertar \ct{self halt}.
...
correspondiente se muestra en el panel inferior. El navegador de procesos no esta equipado con mini-inspectores para \self y \lct{thisContext}, pero \actclick{ing} en los frames de la pila proporciona funcionalidad equivalente.
...
clases son almacenados en un Change Set \ct{change set}. Esto incluye crear
...
no aparecer\'a en el archivo \subind{file}{change set}.
\index{change sorter}

En cualquier momento,existen muchos Change sets, pero solo uno de
ellos\,---\,\ct{ChangeSet current}\,---\,est\'a juntando los cambios que estan
siendo realizados en la imagen. Puedes ver cu\'al Change Set esta
activo y examinar todos los conjuntos de cambios usando Change Sorter, disponible al seleccionar \menu{World \go Tools \ldots \go Change Sorter}.
...
\figref{changeSorter} muestra este navegador.  La barra de t\'itulos muestra que
...
menu \actclick{ing} te permite configurar a un change set distinto como activo,
...
set haciendo \actclick{ing} en los items corrrespondientes.
...
de un set a otro, como se muestra \actclick{ing} en
\figref{changeSorter}. Tambi\'en es posible copiar m\'etodos individuales de un lado a otro.
...
\actclick{ing} en cualquier change set, clase o m\'etodo en cualquier navegador.
...
\ind{Monticello}, es que no soportan la noci\'on de dependencias. Un change set
...
de archivos local, pero una vista t\'ipica es mostrada en \figref{fileList}.
...
los botones depende del nombre del archivo seleccionado en el navegador. En
 ...
 archivo, y para importar \button{filein} el c\'odigo al change set
 \emph{actual}. Podr\'ias pensar que el bot\'on \button{conflicts} te
...
El bot\'on \button{code} es quiz\'as el m\'as \'util para trabajar con change
...
\figref{fileContentsBrowser}) te mostrar\'a las diferencias entre la versi\'on
...
\dothis{Para romper \pharo maliciosamente, intenta \ct{Object become: nil}.}
...
<<<<<<< HEAD
Esto dar\'a la oportunidad de decidir cu\'an atr\'as en el tiempo deseas
navegar.Normalmente, basta con navegar por los cambios tan atr\'as como est\'e
la \'ultima captura. (Puedes obtener el mismo efecto editando \ct{ChangeList
browseRecent: 2000} de manera que el n\'umero \ct{2000} se convierta en otra
cosa, usando prueba y error.)

Una vez que tienes un navegador con los \emph{cambios recientes}, mostrando,
digamos, cambios tan antiguos como tu \'ultima captura, tendr\'as la lista de
todo lo que hayas hecho a \pharo durante ese tiempo. Puedes borrar elementos
de esa lista mediante \actclick{ing}. Cuando est\'es satisfecho, puedes importar
lo que es resta, incorporando as\'i los cambios a tu nueva imagen. Es una
buena idea empezar un nuevo change set, usando el navegador de change set
ordinario, antes de hacer la importaci\'on,de modo que todo tu c\'odigo
recuperado estar\'a en un nuevo change set. Luego puedes exportar este change
set.

¿Una cosa \'util de hacer en el navegador de \emph{cambios recientes} es
 \menu{remove doIts}. Usualmente, no querr\'as importar (y de este modo
 re-ejecutar) doIts. De todos modos, hay una excepci\'on. La creaci\'on de una
 clase se muestra como un \menu{doIt}.
\emph{Antes de que puedas importar los m\'etodos para un clase,la clase debe
existir.} Entonces, si has creado nuevas clases,  \emph{primero} importa
de los doIts de crecaci\'on de clases, luego \menu{remove doIts} e importa
de los m\'etodos.
\lr{Tal vez mecionar que los renombramientos de clases no se archivan y arruinan
completamente el mecanismo del change set (p. 174)} 

======= 

Esto te dar\'a la oportunidad para decidir cu\'anto tiempo atr\'as en la historia deseas examinar. Normalmente, basta con examinar los cambios hasta el \'ultimo snapshot. (Puedes conseguir el mismo efecto mediante la edici\'on de \ct{ChangeList browseRecent: 2000} por lo que el n\'umero \ct{2000} se vuelve algo m\'as, mediante prueba y error.)
...
\begin{itemize}
  ...
  \item El \emph{inspector} y el \emph{explorador} son dos herramientas que son \'utiles para explorar e interactuar con objetos vivos en tu imagen.
      Incluso puedes inspeccionar herramientas por \metaclick{ing} para iniciar
  ...
  debugger es especialmente efectivo como una herramienta de apoyo a las primeras pruebas de desarrollo junto a SUnit (\charef{SUnit}).
  ...
  navegar por el sistema de archivos. Tambi\'en te permite hacer \menu{filein}
