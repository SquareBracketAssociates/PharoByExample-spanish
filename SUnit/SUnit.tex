...
correr\'an. Aqu\'i el contexto es descripto por las dos variables de instancia \ct{full} and \ct{empty}
...
que contiene 5 elementos deber\'ia retornar \ct{verdadero -true}.  Claramente, esta prueba depende del que el
...
de ~5 en el conjunto \ct{lleno} es igual a uno, a\'un cuando agregamos otro elemento ~5 al set.
...
(\ie  subclasses of \ct{TestCase}); cuando algunas de esttas categor\'ias es seleccionada, las clases de pruebas
...
as\'i las subclases de \ct{ClassTestCase} estan indentadas m\'as que las subclases de \ct{Testcase}.
...
\dothis{Abre un Test Runner, selecciona la categor\'ia \menu{MyTest}, y haz click en el bot\'on \button{Run Selected}.}
...
Un recurso es asociado a una sublcase en particular de \ct{TestCase}  sobreescribiendo el m\'etodo de \emph{clase}
\emph{resources}.
...
Puedes elegir cualquiera para reportar con s\'olo sobreescribir \cmind{TestCase}{isLogging} en tu clase de prueba;
tambi\'en deber\'as elegir d\'onde reportar sobreescribiendo  \cmind{TestCase}{failureLog} para
...
\ct{assert: p description: s} es quievalente a \ct{assert: p description: s resumable: false}.
...
la expresi\'on \ct{MyTestCase buildSuiteFromSelectors} retirna un set que contiene todas las 
...
las clases  \ct{TestCase}, \ct{TestResult}, \ct{TestSuite}
y \lct{TestResources}. Por \'ultimo, profundizamos en \sunit siguiendo la ejecuci\'on de una prueba y de un
conjunto de pruebas.
...
\begin{itemize}
  ...
  \item Ussa los m\'etodos de \ct{TestCase} :  \ct{assert:}, \ct{deny:} y otros para hacer aserciones.
